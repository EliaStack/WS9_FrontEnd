...
function EditTask() {
  ...
  useEffect(() => {
    get('api/projet/' + project._id).then((response) => {
      setTitle(response.data.title);
      setDescription(response.data.description);
      setStartAt(
        response.data.startAt ? response.data.startAt.split('T')[0] : '',
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      //Appel API
      await patch('api/projet/' + project._id, {
        title,
        description,
        startAt,
        endAt,
            ...
            return (
              ...
              >
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-gray-400 text-xs">{userObj.email}</span>
                </div>
