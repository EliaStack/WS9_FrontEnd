...
function Project({ project, onUpdate }) {
  ...
  return (
    <div
      key={project._id}
      className="bg-gray-50 dark:bg-gray-800 p-5 rounded-xl border border-gray-400 dark:border-gray-600 border-l-[6px] border-l-blue-600 shadow-[0_4px_20px_rgba(0,0,0,0.08)] hover:shadow-[0_8px_30px_rgba(0,0,0,0.12)] transition-all duration-300 mb-6 flex flex-col md:flex-row gap-4 md:items-center"
    >
      {/* Colonne gauche */}
      <Link
        to="/tasks"
        state={{ projectId: project._id, projectTitle: project.title }}
        className="flex-1 block hover:bg-gray-100/50 dark:hover:bg-gray-700/50 p-2 rounded-lg transition-colors cursor-pointer min-w-0"
      >
        <div className="flex items-center gap-2 mb-2">
          <span
            className={`text-[10px] font-bold px-2.5 py-1 rounded-full uppercase tracking-wide ${
              project.status?.trim().toLowerCase() === 'actif'
      ...
      <div
        className={`grid gap-2 w-full md:grid-cols-1 md:w-auto md:min-w-[120px] ${
      ...
      <div className="flex items-center gap-4 border-t md:border-t-0 md:border-l border-gray-300 dark:border-gray-600 pt-4 md:pt-0 md:pl-4 w-full md:min-w-[150px] md:w-auto">
        <div className="flex flex-col text-left md:text-right w-full">
          <span className="text-xs font-bold text-black dark:text-gray-200 uppercase tracking-wide">
            Responsable :
          </span>
          ...
          <div className="flex flex-col">
            {project.members && project.members.length > 0 ? (
              ...
              <span className="text-sm font-semibold text-gray-800 dark:text-gray-300 italic">
                Personne
              </span>
            )}
          </div>
